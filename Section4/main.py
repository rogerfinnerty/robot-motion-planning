"""
Main entry point for Section 4. This file contains functions to test the graph search and
two-link manipulator path planning functionalities, as well as to visualize the results in
both configuration space and workspace. The main function demonstrates how to use these
functionalities and save the resulting plots and animations.
"""
...
def graph_search_test(save_img=False, outname="assets/graph_search_test.gif"):
    ...
    if save_img and images:
        ...
        save_gif_from_images(images, gif_path, duration=0.1)
...
def two_link_search(
    theta_start: np.ndarray, theta_goal: np.ndarray, outname="assets/twolink_solved.gif"
):
    """
    ...
    ax_config.scatter(
        theta_start[0, 0], theta_start[1, 0], color="g", s=80, marker="o", label="start"
    )
    ax_config.scatter(
        theta_goal[0, 0], theta_goal[1, 0], color="m", s=80, marker="*", label="goal"
    )
    ax_config.set_xlabel(r"$\theta_1$ (rad)")
    ax_config.set_ylabel(r"$\theta_2$ (rad)")
    ax_config.set_title("Two-link configuration space")
