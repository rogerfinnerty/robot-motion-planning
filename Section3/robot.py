...
class TwoLink:
    ...
    def plot(self, theta, color, axes=None):
        """
    ...
    def animate(self, theta, save_gif=False, outfile: str = 'assets/twolink_animation.gif', axes=None):
        """
        ...
        if save_gif:
            ...
            # Determine whether we're drawing into a provided axes or creating our own
            if axes is None:
            ...
            # Capture frames (decimate for shorter GIF)
            frame_step = 5
            pause = 0.05
            for i_theta in range(0, theta_steps, frame_step):

                pre_patches = list(ax.patches)
                pre_lines = list(ax.lines)
                pre_collections = list(ax.collections)
                pre_artists = list(ax.artists)

                self.plot(theta[:, [i_theta]], 'k', axes=ax)
                ...
                if not created_fig:
                    added = [p for p in ax.patches if p not in pre_patches]
                    added += [l for l in ax.lines if l not in pre_lines]
                    added += [c for c in ax.collections if c not in pre_collections]
                    added += [a for a in ax.artists if a not in pre_artists]
                    for art in added:
                        try:
                            art.remove()
                        except Exception:
...
class TwoLinkPotential:
    ...
    xPath (e.g., use  xPath(:,1:5:end)). To avoid clutter, plot a different figure for each start.
        ...
        for start in range(0, nb_starts):
            ...
            print(f"Running planner for start {start+1}/{nb_starts}: ({theta_start[0,0]:.2f}, {theta_start[1,0]:.2f})")
            theta_path, u_path = planner.run(theta_start)

            # Plots
            _, axes = plt.subplots(ncols=2)
            axes[0].set_aspect('equal', adjustable='box')
            sphere_world.plot(axes=axes[0])
            axes[1].plot(u_path.T)
            two_link.animate(theta_path, save_gif, outfile=f'assets/twolink_animation_start{start}.gif', axes=axes[0])
